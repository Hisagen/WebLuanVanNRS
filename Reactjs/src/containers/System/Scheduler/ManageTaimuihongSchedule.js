...
class ManageTaimuihongSchedule extends Component {
  constructor(props) {
    super(props);
    this.state = {
      listDoctors: [],
  ...
  componentDidMount() {
    this.props.fetchAllVienchucChuyenkhoaTaimuihong();
    this.props.fetchAllBuoi();
    this.props.fetchAllPhongChuyenkhoaTaimuihong();
    this.hamdata();
  }

  hamdata = async () => {
    {
      let mang1 = await getAllChuyenkhoaLichbacsikhamService(26);
      let tam = [];
      let tam2 = "";
      let giobd = "";
      let giokt = "";
...
const mapStateToProps = (state) => {
  return {
    ...
    allDoctors: state.admin.vienchucchuyenkhoataimuihongs,
    allScheduleTime: state.admin.buois,
    allPhongs: state.admin.phongchuyenkhoataimuihongs,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    fetchAllVienchucChuyenkhoaTaimuihong: () =>
      dispatch(actions.fetchVienchucChuyenkhoaTaimuihongStart()),
    fetchAllBuoi: () => dispatch(actions.fetchAllBuoiStart()),
    fetchAllPhongChuyenkhoaTaimuihong: () =>
      dispatch(actions.fetchAllPhongChuyenkhoaTaimuihongStart()),
  };
};
