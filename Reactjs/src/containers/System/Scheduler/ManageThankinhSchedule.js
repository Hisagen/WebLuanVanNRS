...
class ManageThankinhSchedule extends Component {
  ...
  async componentDidMount() {
    ...
    // this.props.fetchAllChuyenkhoaThankinhLichbacsikham();
    // this.setState({
    //     mang: await getAllChuyenkhoaLichbacsikhamService(1),
    // })
    // let a = await getAllChuyenkhoaLichbacsikhamService(1);
    // this.state.mang.data && this.state.mang.data.length > 0 &&
    //     this.state.mang.data.map((item, index) => {
    //         this.setState({
    //             data: [
    //                 {
    //                     startDate: moment(+ item.ngay).format('YYYY-MM-DD') + 'T' + item.lichbacsikhambuoi.tenkhungio.substring(0, 5),
    //                     endDate: moment(+ item.ngay).format('YYYY-MM-DD') + 'T' + item.lichbacsikhambuoi.tenkhungio.substring(8, 14),
    //                     title: item.lichbacsikhamvienchuc.hoten + ' - ' + item.lichbacsikhamphong.tenphong
    //                 },
    //             ],
  ...
  hamdata = async () => {
    {
      let mang1 = await getAllChuyenkhoaLichbacsikhamService(22);
      let tam = [];
      let tam2 = "";
      let giobd = "";
      let giokt = "";
  ...
  handleOnchangeDatePicker = async (date) => {
    ...
    await getPhongChuyenkhoaIdService(22, formatedDate);
  };
  ...
  render() {
    ...
    const currentDate2 = "2022-05-06";
    const schedulerData2 = [
      {
        startDate: "2022-05-06T09:45",
        endDate: "2022-05-06T11:00",
        title: "Meeting",
      },
      {
        startDate: "2022-05-07T12:00",
        endDate: "2022-05-08T13:30",
        title: "Go to a gym",
      },
    ];
...
const mapStateToProps = (state) => {
  return {
    ...
    // allLichbacsikhams: state.admin.chuyenkhoathankinhlichbacsikhams
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    fetchAllVienchucChuyenkhoaThankinh: () =>
      dispatch(actions.fetchVienchucChuyenkhoaThankinhStart()),
    fetchAllBuoi: () => dispatch(actions.fetchAllBuoiStart()),
    fetchAllPhongChuyenkhoaThankinh: () =>
      dispatch(actions.fetchAllPhongChuyenkhoaThankinhStart()),
  };
};
