...
class ManageTimmachSchedule extends Component {
  constructor(props) {
    super(props);
    this.state = {
      listDoctors: [],
  ...
  componentDidMount() {
    this.props.fetchAllVienchucChuyenkhoaTimmach();
    this.props.fetchAllBuoi();
    this.props.fetchAllPhongChuyenkhoaTimmach();
    this.hamdata();
  }

  hamdata = async () => {
    {
      let mang1 = await getAllChuyenkhoaLichbacsikhamService(25);
      let tam = [];
      let tam2 = "";
      let giobd = "";
      let giokt = "";
...
const mapStateToProps = (state) => {
  return {
    ...
    allDoctors: state.admin.vienchucchuyenkhoatimmachs,
    allScheduleTime: state.admin.buois,
    allPhongs: state.admin.phongchuyenkhoatimmachs,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    fetchAllVienchucChuyenkhoaTimmach: () =>
      dispatch(actions.fetchVienchucChuyenkhoaTimmachStart()),
    fetchAllBuoi: () => dispatch(actions.fetchAllBuoiStart()),
    fetchAllPhongChuyenkhoaTimmach: () =>
      dispatch(actions.fetchAllPhongChuyenkhoaTimmachStart()),
  };
};
